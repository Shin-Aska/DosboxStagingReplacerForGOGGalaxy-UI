...
if (-Not (Test-Path $exePath)) {
    [System.Windows.MessageBox]::Show("DosboxStagingReplacer.exe not found in the current directory. Please download that first and place it at the same directory as this script is executed.", "Error", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Error)
}
else {
    ...
    function FillGameSelection {
        ...
        if (-not (Test-Path $exePath)) {
            Show-LinkDialog -Message "DosboxStagingReplacer.exe not found." `
                -Url "https://github.com/Shin-Aska/DosboxStagingReplacerForGOGGalaxy/releases/latest" `
